...
static void removeEmployee(InputOutput io) {
    ...
    if (employee != null) {
        io.writeLine("Employee has been deleted " + employee);
    } else {
        io.writeLine("Employee not found");
    }
}
...
static void getDepartmentBudget(InputOutput io) {
	String department = io.readStringOptions("Enter department " + departments, "Wrong department", departments);
	 io.writeLine("Department budget is  " + company.getDepartmentBudget(department));
}

static void getDepartments(InputOutput io) {
	String[] departmentsArray =company.getDepartments();
	io.writeLine ("We have employees in next department : " + String.join(", ", departmentsArray));
}
...
Manager[] managers = company.getManagersWithMostFactor();
    ...
    {
        String result = Arrays.stream(managers)
                .map(Manager::toString)
                .collect(Collectors.joining(", ")); 
        io.writeLine("Managers With Most Factor: " + result);
    }
}
}